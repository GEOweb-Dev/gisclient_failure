...
if($popoint!=null) {
  $sql = "select id_nodo from grafo.nodi where ST_DISTANCE('$popoint', $geom) <".floatVal($_REQUEST["distance"])." ORDER BY ST_DISTANCE('$popoint',$geom) LIMIT 1;";
  $stmt = $db->prepare($sql);
  $stmt->execute();
...
/*
//SENZA INCLUDI I NODI
$joinFilter =  ($excludeVertex) ? "(sg.da_tipo<>'altro' AND sg.da_nodo NOT IN ($excludeVertex)) OR (sg.a_tipo<>'altro' AND sg.a_nodo NOT IN ($excludeVertex))"
...
if($flag == 1) {
  ...
  /*
  //SENZA INCLUDI I NODI...
  $sql.= ($excludeVertex) ? " AND ((sg.da_tipo='altro' OR sg.da_nodo IN ($excludeVertex)) AND (sg.a_tipo='altro' OR sg.a_nodo IN ($excludeVertex)));" 
  ...
  */
  ...
  if(!$selectedNextPipe) {
    $sql="SELECT id_arco,da_nodo,a_nodo, da_tipo,a_tipo FROM grafo.archi WHERE id_arco = $selectedPipe UNION "
...
/*
//SENZA INCLUDI I NODI...
    $sql.= ($excludeVertex) ? " AND  ((g.a_nodo=sg.da_nodo AND (g.a_tipo='altro' OR g.a_nodo IN ($excludeVertex))) OR (g.da_nodo=sg.a_nodo AND (g.da_tipo='altro' OR "
      ."g.da_nodo IN ($excludeVertex))) OR (g.a_nodo=sg.a_nodo AND (g.a_tipo='altro' OR g.a_nodo IN ($excludeVertex))) OR (g.da_nodo=sg.da_nodo AND (g.da_tipo='altro' OR "
      ."g.da_nodo IN ($excludeVertex))));" : " AND ((g.a_nodo=sg.da_nodo AND g.a_tipo='altro') OR (g.da_nodo=sg.a_nodo AND g.da_tipo='altro') OR "
  ...
  if(!empty($idList)){
    ...
    if(strcmp($key,"altro")!=0) {
      ...
      foreach($ELEMENTS[$key]["featureType"]["properties"] as $field)
        $fields[]=$field["name"];
      //error_log("KAPIT:".$excludeRequested);
