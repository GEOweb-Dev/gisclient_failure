  ...
  function reportComponenti($db, $arr, $schema) {
    ...
    foreach($arr as $jj) {
      ...
      if(substr($jj->tipo, 0, strlen("valvola")) == "valvola") {
	      $sql = "select distinct concat('fcl_h_isolation_device_',a.id_tipologia) as id_classe," 
		      ."b.sap_id_poz as sap_id, "
		      ."'".$jj->tipo."' as descrizione, "
			."case when a.id_territorio=1 and b.sap_id_valvola not ilike 'C%' then a.codice_valvola "
			."when a.id_territorio<>1 and b.sap_id_valvola not ilike 'C%' then null "
			//."else substr(a.codice_valvola,1,5) || '-' || a.codice_oggetto end as codice_oggetto "
			."else null end as codice_oggetto "
        ...
        $resultValvola .= (empty($dalmino) ? "" : $dalmino."\r\n"); 
        $dalmino ="";
      }
