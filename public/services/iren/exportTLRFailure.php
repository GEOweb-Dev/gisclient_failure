    ...
    foreach($arr as $jj) {
      ...
      if(substr($jj->tipo, 0, strlen("valvola")) == "valvola") {
	      $sql = "select distinct concat('fcl_h_isolation_device_',a.id_tipologia) as id_classe," 
		      // ."case when a.id_Territorio=1 then b.sap_id_poz else (left(b.sap_id_poz,-1) || 'X') end as sap_id, "
		      ."b.sap_id_poz as sap_id, "
		      ."'".$jj->tipo."' as descrizione, case when a.id_territorio=1 then a.codice_valvola else null end as codice_oggetto "
